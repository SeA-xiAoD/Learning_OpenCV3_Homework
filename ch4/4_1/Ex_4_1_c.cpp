...
void help()
{
	...
	cout << "Arrow keys to control box's position" << endl;
}

void draw_box(cv::Mat& display, unsigned row_pos, unsigned col_pos)
{
	// top and down
	for(int i = 0; i < 10; i++)
	{
		display.at<uchar>(0 + 20*row_pos, i + 10*col_pos) = 255;
		display.at<uchar>(19 + 20*row_pos, i + 10*col_pos) = 255;
	}
	// left and right
	for(int i = 1; i < 19; i++)
	{
		display.at<uchar>(i + 20*row_pos, 0 + 10*col_pos) = 255;
		display.at<uchar>(i + 20*row_pos, 9 + 10*col_pos) = 255;
	}
}

void erase_box(cv::Mat& display, unsigned row_pos, unsigned col_pos)
{
	// top and down
	for(int i = 0; i < 10; i++)
	{
		display.at<uchar>(0 + 20*row_pos, i + 10*col_pos) = 0;
		display.at<uchar>(19 + 20*row_pos, i + 10*col_pos) = 0;
	}
	// left and right
	for(int i = 1; i < 19; i++)
	{
		display.at<uchar>(i + 20*row_pos, 0 + 10*col_pos) = 0;
		display.at<uchar>(i + 20*row_pos, 9 + 10*col_pos) = 0;
	}
}
...
int main( int argc, char** argv) 
{
	...
	// show first display
	draw_box(display, row_count, col_count);
	...
	do
	{
		...
		case '8':
		case '9':
			numbers[ch - '0'].copyTo(display.rowRange(0 + 20*row_count, 20 + \
				20 * row_count).colRange(0 + 10*col_count, 10 + 10*col_count));
			cout << ch << endl;
			erase_box(display, row_count, col_count);
			col_count++;
			break;
		case ' ':
			display.rowRange(0 + 20*row_count, 20 + 20 * row_count).colRange(0 \
				+ 10*col_count, 10 + 10*col_count) = 0;
			erase_box(display, row_count, col_count);
			col_count++;
			break;
		case 13:
